...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		TCHAR szBuf[256];
		swprintf(szBuf, 256, L"correct num : %d", g_nNum);
		OutputDebugString(szBuf);

		CreateWindow(L"static", L"숫자를 입력하세요.", WS_CHILD | WS_VISIBLE, 0, 40, 150, 25, hWnd, (HMENU)-1, hInst, NULL);

		CreateWindow(L"edit", NULL, WS_CHILD | WS_VISIBLE | WS_BORDER, 0, 60, 100, 25, hWnd, (HMENU)3001, hInst, NULL);
		
		CreateWindow(L"button", L"숫자 맞추기", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 0, 0, 100, 25, hWnd, (HMENU)4001, hInst, NULL);
	}
		break;
    case WM_COMMAND:
        {
			...
			case 4001:
			{
				...
				nInputVal = _wtoi(szBuf);

				if (nInputVal == g_nNum) {
					MessageBox(hWnd, L"정답입니다.", L"", MB_OK);
				}
				else if (nInputVal > g_nNum) {
