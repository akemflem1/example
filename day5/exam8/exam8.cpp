...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    switch (message)
    {
		case WM_CREATE:
			{
			CreateWindow(L"edit", NULL, WS_CHILD | WS_VISIBLE | WS_BORDER, 10, 10, 100, 25, hWnd, (HMENU)3001, hInst, NULL);

			CreateWindow(L"button", L"OK", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 120, 10, 100, 25, hWnd, (HMENU)4001, hInst, NULL);
			}
			break;
    case WM_COMMAND:
        {
			...
			case 4001:
			{
				...
				if (nNum>10 && nNum<20 ) {
					swprintf(szBuf, 256, L"%d은(는) 10에서 20사이의 값입니다.", nNum);
					TextOut(hdc,0,120,szBuf ,wcslen(szBuf));
				}
				else {
					swprintf(szBuf, 256, L"%d은(는) 10에서 20사이의 값이 아닙니다.", nNum);
					TextOut(hdc, 0, 120, szBuf, wcslen(szBuf));
				}
				ReleaseDC(hWnd, hdc);
			}
