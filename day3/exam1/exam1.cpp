...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_MOUSEMOVE:
	{
		...
		OutputDebugString(szBuf);		//x좌표         y좌표

		HDC hdc = GetDC(hWnd);
		Rectangle(hdc, 100, 100, 400, 150);
	...
	case WM_LBUTTONDOWN:
	{
		...
		TextOut(hdc, 120, 170, szBuf, wcslen(szBuf) - 1);
		ReleaseDC(hWnd, hdc);
	}
		break;
	...
	case WM_LBUTTONUP:
	{
		...
		HDC hdc = GetDC(hWnd);
		Rectangle(hdc, 100, 200, 400, 250);
		TextOut(hdc, 120, 220, szBuf, wcslen(szBuf) - 1);
		ReleaseDC(hWnd, hdc);
	}
	break;
