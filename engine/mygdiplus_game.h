...
namespace myGdiplusGame {
	...
	void drawTile(Graphics *pGrp, Image *pImgBasicTile, int nPosX, int nPosY, int *pMap)
	{
		...
		pGrp->DrawImage(pImgBasicTile, Rect(nPosX * 16, nPosY * 16, g_nTileSize, g_nTileSize),	g_nTileSize * (nTileIndex % g_nTileXCount), g_nTileSize * (nTileIndex / g_nTileXCount), g_nTileSize, g_nTileSize, UnitPixel);
	}
